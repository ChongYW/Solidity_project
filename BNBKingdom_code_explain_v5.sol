/**
*   ? = Not sure.
*   ?NotSure? = Not sure about the explaination.
*/
...
// CYW: The main logic of the code?NotSure?
...
// CYW: Creating some variable with value?NotSure?
    /* base parameters */
...
// CYW: Initializes the state variables, reverted if the requirement is not meet?NotSure?
...
// CYW: A function that need to check the current user is eligible to compound their rewards under certain conditions when the `contractStarted` is true?NotSure?
...
//      with certain tax implications depending on the user's daily compound bonus and blacklist status?NotSure?
...
//      with provisions for referrals and various calculations related to rewards and deposits?NotSure?
...
// CYW: This Solidity code creates a function that lets users send BNB to the contract. The contract can receive BNB but doesn't do anything with it?NotSure?
...
// CYW: Return the related data(Trade value?) after done some calculations?NotSure?
...
// CYW: Pass the data one by one into different function parameter to calculate and return value(trade value?) and the net worth of the day?NotSure?
...
// CYW: Get related data(trade value?) from `calculateTrade` by given different parameter?NotSure?
...
// CYW: Return the ??? value of sender?NotSure?
    function getMyEggs() public view returns(uint256){
        return users[msg.sender].claimedEggs.add(getEggsSinceLastHatch(msg.sender));
    }

// CYW: Return the ??? value after some calculations by giving the target address?NotSure?
...
// CYW: Only `owner` can transfer the balance to their address?NotSure?
